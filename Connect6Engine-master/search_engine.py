from tools import make_move, is_win_by_premove
...
class SearchEngine:
    ...
    def check_first_move(self):
        """
        Instead of checking the first move by looping through all the possible board
        check only the middle point.
        """
        center_of_the_board = self.m_board[len(self.m_board) // 2][len(self.m_board[0]) // 2]
        if center_of_the_board != Defines.NOSTONE:
            return False
        return True
