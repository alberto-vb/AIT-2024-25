...
def is_win_by_premove(board, preMove):
    ...
    board_full = True  # Variable to track if the board is full
    ...
    if any(Defines.NOSTONE in row for row in board):
...
def is_end_game(board):
    # Check if the board has a 6 in a row in the horizontal direction.
    for i in range(1, Defines.GRID_NUM - 1):
        count = 0
        for j in range(1, Defines.GRID_NUM - 1):
    ...
    # Check if the board has a 6 in a row in the vertical direction.
    for j in range(1, Defines.GRID_NUM - 1):
        count = 0
        for i in range(1, Defines.GRID_NUM - 1):
            if board[i][j] == Defines.BLACK:
    ...
    # Check if the board has a 6 in a row in the diagonal direction.
    for i in range(1, Defines.GRID_NUM - 1):
        count = 0
        for j in range(1, Defines.GRID_NUM - 1):
            if board[i][j] == Defines.BLACK:
...
def move2msg(move):
    """
    Given a move object in number format, return the move in string format, from A to S.
    """
