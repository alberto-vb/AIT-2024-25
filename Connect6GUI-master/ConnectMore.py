...
class App(tkinter.Frame):
    ...
    def __init__(self, master=None):
        tkinter.Frame.__init__(self, master, width=640, height=700)
        self.pack();
    ...
    def initResource(self):
        ...
        self.images = {};
        im = self.images;
        im['go_u'] = tkinter.PhotoImage(file='imgs/go_u.gif');
        im['go_ul'] = tkinter.PhotoImage(file='imgs/go_ul.gif');
        im['go_ur'] = tkinter.PhotoImage(file='imgs/go_ur.gif');
        im['go'] = tkinter.PhotoImage(file='imgs/go.gif');
        im['go_l'] = tkinter.PhotoImage(file='imgs/go_l.gif');
        im['go_r'] = tkinter.PhotoImage(file='imgs/go_r.gif');
        im['go_d'] = tkinter.PhotoImage(file='imgs/go_d.gif');
        im['go_dl'] = tkinter.PhotoImage(file='imgs/go_dl.gif');
        im['go_dr'] = tkinter.PhotoImage(file='imgs/go_dr.gif');
        im['go_-'] = tkinter.PhotoImage(file='imgs/go_-.gif');
        im['go_b'] = tkinter.PhotoImage(file='imgs/go_b.gif');
        im['go_w'] = tkinter.PhotoImage(file='imgs/go_w.gif');
        im['go_bt'] = tkinter.PhotoImage(file='imgs/go_bt.gif');
        im['go_wt'] = tkinter.PhotoImage(file='imgs/go_wt.gif');

        im['angel'] = tkinter.PhotoImage(file='imgs/Emotes-face-angel.gif');
        im['laugh'] = tkinter.PhotoImage(file='imgs/Emotes-face-laugh.gif');
        im['plain'] = tkinter.PhotoImage(file='imgs/Emotes-face-plain.gif');
        im['raspberry'] = tkinter.PhotoImage(file='imgs/Emotes-face-raspberry.gif');
        im['sad'] = tkinter.PhotoImage(file='imgs/Emotes-face-sad.gif');
        im['smile'] = tkinter.PhotoImage(file='imgs/Emotes-face-smile.gif');
        im['smile-big'] = tkinter.PhotoImage(file='imgs/Emotes-face-smile-big.gif');
        im['surprise'] = tkinter.PhotoImage(file='imgs/Emotes-face-surprise.gif');
        im['uncertain'] = tkinter.PhotoImage(file='imgs/Emotes-face-uncertain.gif');
        im['wink'] = tkinter.PhotoImage(file='imgs/Emotes-face-wink.gif');
        ...
        # Widgets
        self.canvas = tkinter.Canvas(self, width=640, height=640);
        self.canvas.pack(side=tkinter.LEFT, fill=tkinter.BOTH, expand=1);
        # Button widgets
        self.controlFrame = tkinter.LabelFrame(self);
        self.controlFrame.pack(fill=tkinter.BOTH, expand=1);
        
        self.controlFrame.aiLevel = labelframe = tkinter.LabelFrame(self.controlFrame, text='AI Level');
        labelframe.pack(fill=tkinter.X, expand=1);
        self.aiLevel = tkinter.IntVar();
        #print(self.aiLevel.get());
        labelframe.lowRBtn = tkinter.Radiobutton(labelframe, text="Low", variable=self.aiLevel, value=4);
        labelframe.lowRBtn.select();
        labelframe.lowRBtn.pack(anchor = tkinter.W);
        labelframe.mediumRBtn = tkinter.Radiobutton(labelframe, text="Medium", variable=self.aiLevel, value=5);
        labelframe.mediumRBtn.pack(anchor = tkinter.W)
        labelframe.highRBtn = tkinter.Radiobutton(labelframe, text="High", variable=self.aiLevel, value=6);
        labelframe.highRBtn.pack(anchor = tkinter.W);
        self.vcf = tkinter.IntVar();
        chbox = tkinter.Checkbutton(labelframe, text ="With VCF", variable = self.vcf, );
        chbox.select();
        chbox.pack(anchor = tkinter.W);
        # print(self.vcf.get());

        self.controlFrame.selectBlack = labelframe = tkinter.LabelFrame(self.controlFrame, text='Black Player');
        labelframe.pack(fill=tkinter.X, expand=1);
        labelframe.blackImg = tkinter.Label(labelframe, image=self.images['go_b']);
        labelframe.blackImg.pack(side=tkinter.LEFT, anchor = tkinter.W);

        self.blackOption = tkinter.IntVar();
        labelframe.humanRBtn = tkinter.Radiobutton(labelframe, text="Human", value=0, variable=self.blackOption, command=self.setBlackHuman);
        labelframe.humanRBtn.select();
        labelframe.humanRBtn.pack(anchor = tkinter.W);
        labelframe.engineRBtn = tkinter.Radiobutton(labelframe, text="AI", value=1, variable=self.blackOption, command=self.setBlackBot);
        labelframe.engineRBtn.pack(anchor = tkinter.W);
        
        self.controlFrame.selectWhite = labelframe = tkinter.LabelFrame(self.controlFrame, text='White Player');
        labelframe.pack(fill=tkinter.X, expand=1);
        labelframe.whiteImg = tkinter.Label(labelframe, image=self.images['go_w']);
        labelframe.whiteImg.pack(side=tkinter.LEFT, anchor = tkinter.W);

        self.whiteOption = tkinter.IntVar();
        labelframe.humanRBtn = tkinter.Radiobutton(labelframe, text="Human", value=0, variable=self.whiteOption, command=self.setWhiteHuman);
        labelframe.humanRBtn.select();
        labelframe.humanRBtn.pack(anchor = tkinter.W);
        labelframe.engineRBtn = tkinter.Radiobutton(labelframe, text="AI", value=1, variable=self.whiteOption, command=self.setWhiteBot);
        labelframe.engineRBtn.pack(anchor = tkinter.W);
        
        self.controlFrame.gameContral = labelframe = tkinter.LabelFrame(self.controlFrame, text='Game Control');
        labelframe.pack(fill=tkinter.X, expand=1);
        labelframe.newBtn = tkinter.Button(labelframe, text='Start Game', command=self.newSingleGame);
        labelframe.newBtn.pack(side=tkinter.TOP, fill=tkinter.X);
        labelframe.backBtn = tkinter.Button(labelframe, text='Back Move', command=self.backMove);
        labelframe.backBtn.pack(fill=tkinter.X);
        #Load engines
        labelframe.loadBtn = tkinter.Button(labelframe, text='Load Black Engine', command=self.loadGameEngineBlack);
        labelframe.loadBtn.pack(fill=tkinter.BOTH);
        labelframe.loadBtn2 = tkinter.Button(labelframe, text='Load White Engine', command=self.loadGameEngineWhite);
        labelframe.loadBtn2.pack(fill=tkinter.BOTH);
        labelframe.quitBtn = tkinter.Button(labelframe, text='Quit Game', command=self.master.destroy);
        labelframe.quitBtn.pack(fill=tkinter.X);
        
        self.controlFrame.tournament = labelframe = tkinter.LabelFrame(self.controlFrame, text='Tournament');
        labelframe.pack(fill=tkinter.X, expand=1);
        labelframe.loadBtn = tkinter.Button(labelframe, text='Load Tournament', command=self.loadTournament);
        labelframe.loadBtn.pack(side=tkinter.TOP, fill=tkinter.X);
        labelframe.newBtn = tkinter.Button(labelframe, text='Start Games', command=self.startTournamentGames);
        labelframe.newBtn.pack(side=tkinter.TOP, fill=tkinter.X);
        labelframe.newBtn = tkinter.Button(labelframe, text='Save results', command=self.saveTournamentGames);
        labelframe.newBtn.pack(side=tkinter.TOP, fill=tkinter.X);
        

        self.controlFrame.aiStatus = labelframe = tkinter.LabelFrame(self.controlFrame, text='AI Status');
        labelframe.pack(side=tkinter.BOTTOM, fill=tkinter.BOTH, expand="yes");
        labelframe.nameBlack = tkinter.Label(labelframe, text='AI B Name');
        labelframe.nameBlack.pack(side=tkinter.TOP, anchor = tkinter.W);
        labelframe.nameWhite = tkinter.Label(labelframe, text='AI W Name');
        labelframe.nameWhite.pack(side=tkinter.TOP, anchor = tkinter.W);
        labelframe.image = tkinter.Label(labelframe, image=self.images['smile']);
        labelframe.image.pack(side=tkinter.TOP, anchor = tkinter.W);
        labelframe.info = tkinter.Label(labelframe, text='');
        labelframe.info.pack(side=tkinter.BOTTOM, anchor = tkinter.W);
    ...
    def createBoardUnit(self, x, y, imageKey):
        lb = tkinter.Label(self.canvas, height=32, width=32);
        lb.x = x;
        lb.y = y;
